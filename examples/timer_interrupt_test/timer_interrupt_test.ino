...
//TIMER2.
timer8	t0(t_settings::alias::T0);
timer16 t1(t_settings::alias::T1);
timer8	t2(t_settings::alias::T2);
timer16 t3(t_settings::alias::T3);
timer16 t4(t_settings::alias::T4);
timer16 t5(t_settings::alias::T5);
...
void setup()
{
	...
	//TIMER0 SETTINGS.
	t0.initialize(t_settings::mode::CTC,
								t_settings::interrupt::COMPA);		// TIMER0_OVF_vect not available (used by arduino).
	t0.setCompareValueA(200);
	t0.setPrescaler(1); 					// Available prescalers for T0: 1, 8, 64, 256, 1024.
	t0.reset();

	//TIMER1 SETTINGS.
	t1.initialize(t_settings::mode::NORMAL,
								t_settings::interrupt::OVF);
	t1.setPrescaler(1); 					// Available prescalers for T1: 1, 8, 64, 256, 1024.
	t1.reset();

	//TIMER2 SETTINGS.
	t2.initialize(t_settings::mode::NORMAL,
								t_settings::interrupt::OVF);
	t2.setPrescaler(32); 					// Available prescalers for T2: 1, 8, 32, 64.
	t2.reset();

	//TIMER3 SETTINGS.
	t3.initialize(t_settings::mode::CTC,
								t_settings::interrupt::COMPB);
	t4.setCompareValueB(2000);
	t3.setPrescaler(64);         				// Available prescalers for T3: 1, 8, 64, 256, 1024.
	t3.reset();

	//TIMER4 SETTINGS.
	t4.initialize(t_settings::mode::CTC,
								t_settings::interrupt::COMPC);
	t4.setCompareValueC(50000);
	t4.setPrescaler(256);         				// Available prescalers for T4: 1, 8, 64, 256, 1024.
	t4.reset();

	//TIMER5 SETTINGS.
	t5.initialize(t_settings::mode::NORMAL,
								t_settings::interrupt::NONE);
